...
namespace UIAssistant
{
    public class TextSettingsWindow : SettingsWindow
    {
        ...
        readonly GUIContent ImportLocalizationSetsContent = new("Import Localization Sets", "Import new, or update existing Localization Sets using data copied from a localization sheet.");
        ...
        readonly GUIContent MissingStringContent = new("Missing String", "Text to be displayed by a Styled Text when its Localization Set is missing.");
        ...
        void LocalizationSets()
        {
            ...
            if (AddButton(ImportLocalizationSetsContent)) InitializeLocalizationImport(GUIUtility.systemCopyBuffer, "\t");
        ...
        void InitializeLocalizationImport(string text, string splitChar)
        {
            ...
            List<string> languageNames = new(rows[0].Split(splitChar));
            ...
            List<int> languageIndexes = new();

            for (int i = 0; i < TextSettings.Languages.Count; i++)
            {
                int languageIndex = -1;

                for (int j = 1; j < languageNames.Count; j++)
                {
                    if (languageNames[j] == TextSettings.Languages[i].Name)
                    {
                        languageIndex = j;
                        break;
                    }
                ...
                languageIndexes.Add(languageIndex);
            }
            ...
            for (int i = 1; i < rows.Count; i++)
            {
                ...
                for (int j = 0; j < TextSettings.Languages.Count; j++)
                {
                    ...
                    if (languageIndexes[j] > -1) localization.Text = localizations[languageIndexes[j]];
